...
const LEGACY_SOURCE_KEYS = new Set(["kind", "locator", "integrity"]);
const LEGACY_TARGET_KEYS = new Set(["altair", "vega", "deneb", "platforms"]);
...
const stringArray = (value: unknown, path: string): readonly string[] | undefined => {
...
const versionRanges = (value: unknown, path: string): Readonly<Record<string, string>> | undefined => {
  if (value === undefined) return undefined;
  if (!isRecord(value)) {
  ...
  for (const [id, rangeValue] of Object.entries(value)) {
    if (!PLUGIN_ID.test(id)) {
      throw new TypeError(`${path} has an invalid plugin ID '${id}'`);
    }
    const range = requireString(rangeValue, `${path}.${id}`);
    if (!isValidVegaSemVerRange(range)) {
      throw new TypeError(`${path}.${id} is not a semantic-version range`);
    }
    output[id] = range;
  }
  return output;
...
export const altairPluginCatalogEntryKey = (entry: Pick<VegaPluginMarketplaceEntry, "id" | "version">): string =>
  `${entry.id}@${entry.version}`;

const parseCanonicalEntry = (value: unknown, path: string): VegaPluginMarketplaceEntry => {
  try {
...
const legacyRegistryPackage = (locator: string, version: string): string =>
  locator.endsWith(`@${version}`) ? locator.slice(0, -(version.length + 1)) : locator;

const legacySource = (value: unknown, version: string, path: string): VegaPluginInstallSource => {
  if (!isRecord(value)) {
    throw new TypeError(`${path} must be an object`);
  }
...
const adaptLegacyEntry = (
  ...
  if (scope !== "authoring" && scope !== "runtime" && scope !== "host") {
  ...
  if (targetsValue !== undefined) {
    ...
    rejectUnknownKeys(targetsValue, LEGACY_TARGET_KEYS, `${path}.targets`);
    targets = {
      ...(targetsValue.altair === undefined
        ? {}
        : {
            altair: requireString(targetsValue.altair, `${path}.targets.altair`),
          }),
      ...(targetsValue.vega === undefined
        ? {}
        : {
            vega: requireString(targetsValue.vega, `${path}.targets.vega`),
          }),
      ...(targetsValue.deneb === undefined
        ? {}
        : {
            deneb: requireString(targetsValue.deneb, `${path}.targets.deneb`),
          }),
      ...(targetsValue.platforms === undefined
        ? {}
        : {
            platforms: stringArray(targetsValue.platforms, `${path}.targets.platforms`)!,
          }),
    };
    for (const range of [targets.altair, targets.vega, targets.deneb]) {
      if (range !== undefined && !isValidVegaSemVerRange(range)) {
        throw new TypeError(`${path}.targets contains an invalid version range`);
      }
    }
  }
  const canonicalTargets: VegaPluginTargets | undefined =
    targets?.platforms?.length || targets?.vega || targets?.deneb
      ? {
          ...(targets.platforms ? { platforms: targets.platforms } : {}),
          ...(targets.vega || targets.deneb
            ? {
                engineVersion: targets.vega ?? targets.deneb,
              }
            : {}),
        }
      : undefined;
  const entry: VegaPluginMarketplaceEntry = {
    ...
    ...(value.capabilities === undefined
      ? {}
      : {
          capabilities: stringArray(value.capabilities, `${path}.capabilities`)!,
        }),
    ...(value.permissions === undefined
      ? {}
      : {
          permissions: stringArray(value.permissions, `${path}.permissions`)!,
        }),
    ...(value.dependencies === undefined
      ? {}
      : {
          dependencies: versionRanges(value.dependencies, `${path}.dependencies`)!,
        }),
    ...(canonicalTargets ? { targets: canonicalTargets } : {}),
    source: legacySource(value.source, version, `${path}.source`),
  };
  return {
    ...
    extension: {
      ...
      ...(value.conflicts === undefined
        ? {}
        : {
            conflicts: versionRanges(value.conflicts, `${path}.conflicts`)!,
          }),
      ...(targets?.altair ? { altairVersion: targets.altair } : {}),
    },
  };
};

const parseAuthoringExtension = (value: unknown, path: string): AltairPluginAuthoringExtension => {
  if (!isRecord(value)) {
    throw new TypeError(`${path} must be an object`);
  }
  rejectUnknownKeys(value, new Set(["scope", "conflicts", "altairVersion", "permissions"]), path);
  let scope: AltairPluginScope | undefined;
  if (value.scope !== undefined) {
    const candidate = requireString(value.scope, `${path}.scope`);
    if (candidate !== "authoring" && candidate !== "runtime" && candidate !== "host") {
      throw new TypeError(`${path}.scope is unsupported`);
    }
    scope = candidate;
  }
  ...
  if (value.altairVersion !== undefined) {
    altairVersion = requireString(value.altairVersion, `${path}.altairVersion`);
    if (!isValidVegaSemVerRange(altairVersion)) {
      throw new TypeError(`${path}.altairVersion is invalid`);
    }
  }
  return {
    ...(scope ? { scope } : {}),
...
const parseExtensions = (value: unknown, path: string): AltairPluginCatalogExtensions | undefined => {
  if (value === undefined) return undefined;
  if (!isRecord(value)) {
    throw new TypeError(`${path} must be an object`);
  }
  ...
  rejectUnknownKeys(value.altair, new Set(["entries"]), `${path}.altair`);
  if (!isRecord(value.altair.entries)) {
    throw new TypeError(`${path}.altair.entries must be an object`);
  }
  return {
    altair: {
      entries: Object.fromEntries(
        Object.entries(value.altair.entries)
          ...
          .map(([key, extension]) => [
            key,
            parseAuthoringExtension(extension, `${path}.altair.entries[${JSON.stringify(key)}]`),
          ]),
      ),
    },
  };
...
export const parseAltairPluginCatalogEntry = (value: unknown, path = "$"): VegaPluginMarketplaceEntry =>
  isRecord(value) && value.format === "vega-plugin-entry"
    ? parseCanonicalEntry(value, path)
    : adaptLegacyEntry(value, path).entry;

export const parseAltairPluginCatalog = (value: unknown, fallbackId = "catalog"): AltairPluginCatalog => {
  const root = Array.isArray(value) ? { plugins: value } : value;
  if (!isRecord(root)) {
    throw new TypeError("Plugin catalog must be an object or array");
  }
  rejectUnknownKeys(root, new Set(["format", "formatVersion", "id", "plugins", "extensions"]), "$");
  if (
    root.format !== undefined &&
    root.format !== ALTAIR_PLUGIN_CATALOG_FORMAT &&
    root.format !== LEGACY_ALTAIR_PLUGIN_CATALOG_FORMAT
  ) {
    throw new TypeError(`$.format must be '${ALTAIR_PLUGIN_CATALOG_FORMAT}'`);
  }
  if (root.formatVersion !== undefined && root.formatVersion !== 1) {
    throw new TypeError("$.formatVersion must be 1");
  }
  if (!Array.isArray(root.plugins)) {
    throw new TypeError("$.plugins must be an array");
  }
  if (root.plugins.length > ALTAIR_PLUGIN_CATALOG_MAX_ENTRIES) {
    throw new RangeError(`Plugin catalog exceeds ${ALTAIR_PLUGIN_CATALOG_MAX_ENTRIES} entries`);
  }
  const extensions = parseExtensions(root.extensions, "$.extensions");
  const authoring = new Map(Object.entries(extensions?.altair?.entries ?? {}));
  const entries = root.plugins.map((candidate, index) => {
    const path = `$.plugins[${index}]`;
    if (isRecord(candidate) && candidate.format === "vega-plugin-entry") {
    ...
    if (existing && JSON.stringify(existing) !== JSON.stringify(adapted.extension)) {
      throw new TypeError(`Conflicting Altair extension metadata for ${key}`);
    }
    authoring.set(key, adapted.extension);
    return adapted.entry;
  });
  const id = root.id === undefined ? fallbackId : requireString(root.id, "$.id");
  ...
  const plugins = new VegaStaticPluginCatalog(entries, id).entries();
  return {
    ...canonical,
    plugins,
    ...(authoring.size
      ? {
          extensions: {
            altair: {
              entries: Object.fromEntries([...authoring].sort(([left], [right]) => left.localeCompare(right))),
            },
          },
        }
      : {}),
...
const defaultCatalogFetch: AltairPluginCatalogFetch = async (url, init) => {
  if (typeof globalThis.fetch !== "function") {
    throw new Error("No fetch implementation is available for HTTP plugin catalogs");
  }
...
const validateCatalogUrl = (value: string): URL => {
  ...
  if (url.protocol !== "http:" && url.protocol !== "https:") {
    throw new TypeError("HTTP plugin catalog URL must use http: or https:");
  }
  if (url.username || url.password) {
    throw new TypeError("HTTP plugin catalog URL must not contain credentials");
  }
  url.hash = "";
  return url;
};
...
export const loadAltairPluginCatalog = async (
  ...
  const response = await (options.fetch ?? defaultCatalogFetch)(url.href, {
    headers: { accept: "application/json" },
    ...(options.signal ? { signal: options.signal } : {}),
  });
  options.signal?.throwIfAborted();
  if (!response.ok) {
    throw new Error(`Plugin catalog request failed with HTTP ${response.status}`);
  }
  const contentType = response.headers?.get("content-type");
  if (contentType && !/(?:application|text)\/(?:[\w.+-]*\+)?json\b/i.test(contentType)) {
    throw new TypeError(`Plugin catalog returned non-JSON content type '${contentType}'`);
  }
  const text = await response.text();
  options.signal?.throwIfAborted();
  if (new TextEncoder().encode(text).byteLength > ALTAIR_PLUGIN_CATALOG_MAX_BYTES) {
    throw new RangeError(`Plugin catalog exceeds ${ALTAIR_PLUGIN_CATALOG_MAX_BYTES} bytes`);
  }
  let metadata: unknown;
  try {
    metadata = JSON.parse(text) as unknown;
  } catch (error) {
    throw new TypeError(`Plugin catalog is not valid JSON: ${error instanceof Error ? error.message : String(error)}`);
  }
  return parseAltairPluginCatalog(metadata, source.id);
};

export class AltairHttpPluginCatalogProvider implements VegaPluginCatalogProvider {
  readonly id: string;
  private readonly source: Extract<AltairPluginCatalogSource, { readonly kind: "http" }>;
  private readonly options: LoadAltairPluginCatalogOptions;

  constructor(
    source: Extract<AltairPluginCatalogSource, { readonly kind: "http" }>,
    options: LoadAltairPluginCatalogOptions = {},
  ) {
    this.id = source.id;
    this.source = source;
  ...
  async entries(): Promise<readonly VegaPluginMarketplaceEntry[]> {
    return (await loadAltairPluginCatalog(this.source, this.options)).plugins;
  }
}

export const mergeAltairPluginCatalogs = (...catalogs: readonly AltairPluginCatalog[]): AltairPluginCatalog => {
  const authoring = new Map<string, AltairPluginAuthoringExtension>();
  for (const catalog of catalogs) {
    for (const [key, extension] of Object.entries(catalog.extensions?.altair?.entries ?? {})) {
      const existing = authoring.get(key);
      if (existing && JSON.stringify(existing) !== JSON.stringify(extension)) {
        throw new Error(`Conflicting Altair extension metadata for ${key}`);
      }
  ...
  return {
    format: ALTAIR_PLUGIN_CATALOG_FORMAT,
    formatVersion: 1,
    id: catalogs.map(({ id }) => id).join("+") || "empty",
    plugins: normalizeVegaPluginCatalog(catalogs.flatMap(({ plugins }) => plugins)),
    ...(authoring.size
      ? {
          extensions: {
            altair: {
...
export const searchAltairPluginCatalog = (
  ...
  query: string,
): readonly VegaPluginMarketplaceEntry[] => searchVegaPluginCatalog(catalog.plugins, { text: query });
...
export const altairPluginAuthoringExtension = (
  ...
  catalog.extensions?.altair?.entries[altairPluginCatalogEntryKey(entry)];
