...
const policies = {
  "@haneoka/altair-plugin-marketplace": {
    repository:
      "git+https://github.com/haneoka-gakuen/altair-plugin-marketplace.git",
    peerDependencies: [
      "@haneoka/altair",
      "@haneoka/vega",
...
for (const path of publishableFiles.filter(
  (entry) => entry.startsWith("dist/") && entry.endsWith(".js"),
)) {
  const builtJavaScript = await readFile(resolve(root, path), "utf8");
  if (
    /\beval\s*\(|\bFunction\s*\(|\bimport\s*\(/u.test(
      builtJavaScript,
    )
  ) {
    fail(
      `marketplace code must not evaluate or dynamically import catalog text (${path})`,
    );
  }
